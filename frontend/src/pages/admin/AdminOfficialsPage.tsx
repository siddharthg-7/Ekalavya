      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-col md:flex-row gap-3 items-center justify-between">
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 border border-slate-300 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent"
          />
        </div>

        <div className="flex items-center gap-2 w-full md:w-auto">
          <label className="text-xs font-medium text-slate-500 whitespace-nowrap">Division:</label>
          <select
            value={departmentFilter}
            onChange={e => setDepartmentFilter(e.target.value)}
            className="w-full md:w-64 px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-slate-900"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredOfficials.map(off => (
            <Link
              ...
              className="group bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:border-slate-400 hover:shadow-md transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-700 font-serif font-bold text-sm">
                      {off.name
                        .split(' ')
                        .map(n => n[0])
                        .join('')}
                    ...
                    <div>
                      <h2 className="font-semibold text-base text-slate-900 group-hover:text-blue-600 transition-colors">
                        {off.name}
                      </h2>
                      <div className="text-xs text-slate-500 font-medium">{off.designation}</div>
                    </div>
                  </div>
                  <span
              ...
              <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                ...
                <span className="inline-flex items-center font-medium text-slate-900 group-hover:text-blue-600 group-hover:translate-x-0.5 transition-all">
