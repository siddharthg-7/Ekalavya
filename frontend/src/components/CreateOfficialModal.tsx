...
import { X, UserPlus, ShieldCheck, Building2, Briefcase, GraduationCap, Award } from 'lucide-react';
...
export const CreateOfficialModal: React.FC<CreateOfficialModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs animate-in fade-in duration-200">
      <div 
        className="bg-white rounded-2xl p-6 sm:p-8 max-w-lg w-full shadow-2xl border border-[#DCE3EA] relative max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-5 right-5 p-2 rounded-full hover:bg-slate-100 transition-colors text-[#52657A] cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex items-center gap-3 mb-6">
          <div className="w-10 h-10 rounded-xl bg-blue-50 text-[#2563D9] flex items-center justify-center border border-blue-100">
            <UserPlus className="w-5 h-5" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-[#102A43]">Register Official</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-lg bg-[#F7F9FC] border border-[#DCE3EA] focus:border-[#2563D9] outline-none text-sm text-[#102A43]"
            />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-[#102A43] mb-1.5 flex items-center gap-1.5">
                <Briefcase className="w-3.5 h-3.5 text-[#E8871A]" />
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-[#102A43] mb-1.5 flex items-center gap-1.5">
                <Building2 className="w-3.5 h-3.5 text-[#E8871A]" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-[#102A43] mb-1.5 flex items-center gap-1.5">
                <Award className="w-3.5 h-3.5 text-[#E8871A]" />
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-[#102A43] mb-1.5 flex items-center gap-1.5">
              <GraduationCap className="w-3.5 h-3.5 text-[#E8871A]" />
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2.5 rounded-lg bg-[#F7F9FC] border border-[#DCE3EA] focus:border-[#2563D9] outline-none text-sm text-[#102A43] resize-none"
            />
          </div>
          ...
          <div className="pt-4 flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 rounded-lg text-xs font-semibold text-[#52657A] hover:bg-slate-100 transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
